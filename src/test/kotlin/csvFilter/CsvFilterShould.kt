...
class CsvFilterShould {
    ...
    fun exclude_files_without_header(){
        ...
        val result = filter.filter(listOf(invoiceLine))
        assertThat(result).isEmpty()
    }

    @Test
    fun return_empty_when_the_file_is_empty(){
        val result = filter.filter(listOf())
        assertThat(result).isEmpty()
    }
    ...
    fun exclude_lines_with_same_number_facture_as_number_facture_must_be_unique(){
        ...
        val result = filter.filter(listOf(headerLine,invoiceLine, invoiceLine2, invoiceLine3, invoiceLine4, invoiceLine5))
